...
class HybridSearch:
    ...
    def weighted_search(self, query, alpha, limit=5):
        ...
        for (normal_score, k_meta, s_meta) in zip(normal_scores, bm25_scores, semantic_scores):
            k_score = k_meta[1]
        ...
        res = sorted(self.doc_hybridized_scores.items(), key=lambda x: x[1]["hybrid_score"], reverse=True)
        if limit <= len(res):
            return res[:limit]
        return res
